...
def make_int_grid(start:int, end:int, rows:int, component:str,
                  columns:int=None) -> array:
    ...
    if component == 'x':    # working with an x component
        if start >= lower_8 and end <= upper_8:
            grid = array([arange(start, end, 2, 
                                 dtype='int8') for _ in range(rows)])
        elif start >= lower_16 and end <= upper_16:
            grid = array([arange(start, end, 2, 
                                 dtype='int16') for _ in range(rows)])
        elif start >= lower_32 and end <= upper_32:
            grid = array([arange(start, end, 2, 
                                 dtype='int32') for _ in range(rows)])
        else:
            grid = array([arange(start, end, 2, 
                                 dtype='int64') for _ in range(rows)])

    elif component == 'y':  # working with an y component
        if start >= lower_8 and end <= upper_8:
            grid = array([linspace(start + row, start + row, columns, 
                                   dtype='int8') for row in range(rows)])
        elif start >= lower_16 and end <= upper_16:
            grid = array([linspace(start + row, start + row, columns, 
                                   dtype='int16') for row in range(rows)])
        elif start >= lower_32 and end <= upper_32:
            grid = array([linspace(start + row, start + row, columns, 
                                   dtype='int32') for row in range(rows)])
        else:
            grid = array([linspace(start + row, start + row, columns, 
                                   dtype='int64') for row in range(rows)])
    return grid


def make_uint_grid(start:int, end:int, rows:int, component:str,
                   columns:int=None) -> array:
    ...
    if component == 'x':
        if start <= lim_8 and end <= lim_8:
            grid = array([arange(start, end, 2, 
                                 dtype='uint8') for _ in range(rows)])
        elif start <= lim_16 and end <= lim_16:
            grid = array([arange(start, end, 2, 
                                 dtype='uint16') for _ in range(rows)])
        elif start <= lim_32 and end <= lim_32:
            grid = array([arange(start, end, 2, 
                                 dtype='uint32') for _ in range(rows)])
        else:
            grid = array([arange(start, end, 2, 
                                 dtype='uint64') for _ in range(rows)])
    elif component == 'y':
        if start <= lim_8 and end <= lim_8:
            grid = array([linspace(start + row, start + row, columns, 
                                   dtype='uint8') for row in range(rows)])
        elif start <= lim_16 and end <= lim_16:
            grid = array([linspace(start + row, start + row, columns, 
                                   dtype='uint16') for row in range(rows)])
        elif start <= lim_32 and end <= lim_32:
            grid = array([linspace(start + row, start + row, columns, 
                                   dtype='uint32') for row in range(rows)])
        else:
            grid= array([linspace(start + row, start + row, columns, 
                                  dtype='uint64') for row in range(rows)])
    return grid